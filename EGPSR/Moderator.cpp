...
void MyController::onInit(InitEvent &evt) {  
	...
	m_rooms.push_back("bed room"); //2
	m_rooms.push_back("lobby"); //3
	m_rooms.push_back("corridor"); //4
...
double MyController::onAction(ActionEvent &evt) {
  ...
  bool available = checkService("Referee");
  if(!available && m_ref != NULL) m_ref = NULL;
  else if(available && m_ref == NULL){
    m_ref = connectToService("Referee");
  }
...
//        pcolState=false;
      ...
      std::string msg = "Referee/Collision with [" + m_entNames[k] + "]" "/-100";
      if(m_ref != NULL){
...
void MyController::onCheckRoom(){
	...
	if(x>100&&x<500&&z>75&&z<425)	num=1; // kitchen
	if(x>-500&&x<-100&&z>75&&z<425) num=2; // bed room
	if(x>-500&&x<-100&&z>-425&&z<-75) num=3; // lobby
	std::string msg = "Referee/Robot is in the " + m_rooms[num];
	LOG_MSG((msg.c_str()));
}
