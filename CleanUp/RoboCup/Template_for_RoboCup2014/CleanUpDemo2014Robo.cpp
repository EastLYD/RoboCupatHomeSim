...
void DemoRobotController::onInit(InitEvent &evt)
{
  ...
  m_frontTrash1     = Vector3d(300, 0.0, -65.0);
...
double DemoRobotController::onAction(ActionEvent &evt)
{
  ...
  case 90: {  // rotate toward the trash
    if(evt.time() >= m_time && m_state==90) {
      ...
      Vector3d l_tpos;
      this->recognizeObjectPosition(l_tpos, m_target);
...
void DemoRobotController::onRecvMsg(RecvMsgEvent &evt)
{
  ...
  if(sender == "moderator_0"){
	...
	if(found != std::string::npos){
		std::string cmd = msg.substr(0, found);
		
		// set the current target name
		if(cmd == "Target_name"){
			m_target = msg.substr(found+1);
		}
	}
...
{
  //std::cout << "m_grasp : " << m_grasp << std::endl;
  ...
  if (m_grasp == false) {
    ...
    //std::cout << "with.size() : " << with.size() << std::endl;
